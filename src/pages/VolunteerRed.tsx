...
const VolunteerRed: React.FC = () => {
  const { data, isLoading, appendScoresRed, deleteScoreRed, appendData } = useDataContext();
  ...
  const handleButtonClick = async (
    ...
    if (
      ...
      playAudio1();
      deleteScoreRed(mission.active_comparison, round);
    } else {
      playAudio();
      appendScoresRed({
  ...
  if(!data?.is_start_matching && !data?.to_submit) {
     return <div className="flex flex-col h-screen items-center justify-center bg-gray-800" >
        <div className="h-full font-anton uppercase flex-col tracking-wide w-full text-white flex justify-center items-center text-3xl" style={{background: "radial-gradient(circle, #EF4444, rgba(31, 41, 55, 0.5))"}}>
    ...
    <div className="flex flex-col">
      {data?.to_submit ? (
        <div className="flex flex-col justify-center items-center h-screen text-2xl bg-gray-800">
          <div
            className="h-full w-full text-white flex justify-center items-center text-3xl"
            style={{
              background:
                "radial-gradient(circle, #EF4444, rgba(31, 41, 55, 0.5))",
            }}
          >
            The match is done
          </div>
        ...
        <div className="flex flex-col">
          ...
          <h1 className="font-anton uppercase text-center mt-4 text-2xl text-red-600">
          ...
          <div className="flex flex-col gap-4 px-4 mt-4">
            {missions.round1.map((mission, index) => (
              <button
                ...
                className={`w-full h-[8vh] grid grid-cols-3 font-anton bg-red-600 text-white rounded-xl px-3 py-1 uppercase leading-9 ${
                  isActive(mission, 1) ? "active" : ""
                }`}
              >
                <div className="flex flex-col justify-start items-start">Obstacle</div>
                <h1 className="text-[2rem] font-bold italic flex justify-center items-center">
                  {mission.obstacle}
                </h1>
                <span className="text-lg flex justify-end items-end">{mission.score} Score</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-4 px-4 mt-4">
            ...
            {token.pairingPoint === '2' ? <button onClick={() => {appendData({is_left_time_stop: true})}} className={`${data?.is_left_time_stop ? "bg-green-600" : "bg-red-600"} w-full mt-10 flex items-center gap-2 justify-center text-[1.75rem] font-anton uppercase text-white rounded-xl py-4 leading-9`}>Final <LuGoal/></button> : ""}
          </div>
        </div>
      )}
    </div>
